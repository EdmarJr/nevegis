...
import br.com.oaks.ICPBravo.algorithm.digest.SHA224;
import br.com.oaks.ICPBravo.algorithm.digest.SHA256;
import br.com.oaks.ICPBravo.algorithm.digest.SHA384;
...
import br.com.oaks.ICPBravo.auth.ICPBravoChallenge;
import br.com.oaks.ICPBravo.auth.ICPBravoResponse;
import br.com.oaks.ICPBravo.auth.ICPBravoCRServer;
...
import br.com.oaks.ICPBravo.cms.CMSSignerInformation.XmlEncapsulated;
...
import br.com.oaks.ICPBravo.cms.pkcs9.Pkcs9Attribute;
...
import br.com.oaks.ICPBravo.manager.ldap.LdapDNFilter;
import br.com.oaks.ICPBravo.manager.ldap.LdapEmailFilter;
import br.com.oaks.ICPBravo.manager.ldap.LdapSerialNumberFilter;
import br.com.oaks.ICPBravo.ntp.NtpClient;
...
//	protected static ICPBravoCertificate certRoot2;
	 ...
	 * 	SymmetricAlgorithm[] algs = {new DESCBC(), new DESECB(),
	 * 	new TripleDES112ECB(), new TripleDES112CBC(), new TripleDES168ECB(), new TripleDES168CBC(),
	 * 	new AES128CBC(), new AES128ECB(), new AES192CBC(), new AES192ECB(), new AES256CBC(), new AES256ECB(),
	 * 	new RC4(),
	 * 	new RC2CBC(), new RC2ECB(),};
	 * 	byte[] bts = &quot;teste&quot;.getBytes();
	 * 	byte[] bts2 = &quot;teste2&quot;.getBytes();
	 * 	byte[] bts3 = &quot;teste3&quot;.getBytes();
	 * 	byte[] btsAll = &quot;testeteste2teste3&quot;.getBytes();
	 * 
	 * 	// É possível ainda setar os parâmetros de inicialização próprios
	 * algs[6].setAditionalStringParameters("/EBC/NOPADDING");
	 * algs[6].setAlgorithmParameterSpec(null);
	 * 
	 ...
	 * 			}
	 * 
	 *			// Teste em partes multiplas
	 *			Cipher cipher = algs[a].createCipher(Cipher.ENCRYPT_MODE, secKey);
	 *			cr = cipher.update(bts);
	 *			cr = br.com.oaks.ICPBravo.util.Arrays.sum(cr, cipher.update(bts2));
	 *			cr = br.com.oaks.ICPBravo.util.Arrays.sum(cr, cipher.update(bts3));
	 *			cr = br.com.oaks.ICPBravo.util.Arrays.sum(cr, cipher.doFinal());
	 *			
	 *			cipher = algs[a].createCipher(Cipher.DECRYPT_MODE, secKey);
	 *			ret = cipher.doFinal(cr);
	 *			if (Arrays.equals(btsAll, ret)) {
	 *				cron2.marcaTotal(algs[a].toString() + ": OK Multiplo");
	 *			} else {
	 *				System.out.println(algs[a].toString() + ": Falha");
	 *			}
	 * 		} catch (Exception e) {
...
Fim de teste de algoritmo assimétrico (121492 ms)
	 ...
	 * 	DigestAlgorithm[] algs = {new MD5(), new SHA1(), new SHA224(), new SHA256(), new SHA384(), new SHA512(), new HMAC_SHA1()};
	 * 	byte[] bts = &quot;teste&quot;.getBytes();
	 * 
	 * 	for (int a = 0; a &lt; algs.length; a++) {
	 * 		try {
...
protected static void testaKEK() {
	...
	try {
		SymmetricAlgorithm sym = new AES128CBC();
...
Fim de Teste de KEK (672 ms)
	...
	*/
	 ...
	 * 	PRNG[] algs = {new LFSR(), new SHA1_PRNG(), new WindowsPRNG()};
	 * 
	 * 	for (int a = 0; a &lt; algs.length; a++) {
	 * 		try {
...
Fim de teste de números aleatórios (31 ms)
	...
	public static void testaNumerosAleatorios() {
		cron.zera();
		System.out.println("Testando geração de números aleatórios");
		PRNGAlgorithm[] algs = {new LFSR(), new SHA1_PRNG(), new WindowsPRNG()};
	...
	public static void testaCertificadoX509v3() {
		...
		try {
			gen = new ICPBravoCertificateGenerator(manager.getProvider(), "Pessoa", BigInteger.valueOf(4), "Pessoa", certRoot, ICPBravoCertificateGenerator.getDefaultKeyUsageForFinal(),
					...
					from, to, 
					new SignatureAlgorithm(new SHA1(), new RSA1024ECB(manager.getProvider())), 
...
... infraestrutura criada. (1181 ms)
	...
	public static void inicializaTeste() {
		...
		try {
			...
			manager = ICPBravoManager.getDefaultICPManager(); // new TestManager("JKS", new ICPBravoProvider(), "1234".toCharArray());
			provider = ICPBravoManager.getICPBravoProvider(); // manager.getProvider();
...
public static ICPBravoCertificate criaCertificadoRaiz(Date from, Date to) throws NoSuchAlgorithmException, NotSuportedAlgorithmException, IOException, ErroKeystoreException, InvalidKeyException, IllegalStateException, SignatureException, KeyStoreException, CertificateException, ChaveInvalidaException, SigningException, ErroIrrecuperavelEmChaveException, NoSuchProviderException, InvalidAlgorithmParameterException, InvalidParameterSpecException {
	...
	return gen.generate(manager, false);
}
	 * </pre>
	...
	private static ICPBravoCertificate criaCertificado1(ICPBravoCertificate root, Date from, Date to) throws NoSuchAlgorithmException, NotSuportedAlgorithmException, IOException, ErroKeystoreException, InvalidKeyException, IllegalStateException, SignatureException, KeyStoreException, CertificateException, ChaveInvalidaException, SigningException, ErroIrrecuperavelEmChaveException, NoSuchProviderException, ManagerNotInitialedException, InvalidAlgorithmParameterException, InvalidParameterSpecException {
		ICPBravoCertificateGenerator gen = new ICPBravoCertificateGenerator(
				...
				from, to, 
				new SignatureAlgorithm(new SHA1(), new RSA1024ECB(provider)), 
	...
	protected static ICPBravoCertificate criaCertificado2(ICPBravoCertificate root, Date from, Date to) throws Exception {
		ICPBravoCertificateGenerator gen = new ICPBravoCertificateGenerator(
				...
				"OU=SRF e-CPF fake,OU=Secretaria da Receita Federal fake - SRFf", null, null, 
				from, to, 
				new SignatureAlgorithm(new SHA1(), new RSA1024ECB(provider2)), 
	...
	protected static ICPBravoCertificate criaCertificado3(ICPBravoCertificate root, Date from, Date to) throws NoSuchAlgorithmException, NotSuportedAlgorithmException, IOException,
		...
		ICPBravoCertificateGenerator gen = new ICPBravoCertificateGenerator(provider, "Pessoa 3", BigInteger.valueOf(4), "Pessoa 3", root, ICPBravoCertificateGenerator
				...
				null, from, to, new SignatureAlgorithm(new SHA1(), new RSA1024ECB(provider)), "pessoa2S@oaktech.com.br", ICPBravoUtil.getDate(
						2009, 1, 1), // nascimento
				"12345678901", // cpf
				null, // nis
				"12345678909", // rg
	...
	public static void testaAssinaturaDePDF() {
		try {
			...
			sd.addSigner(cert1, new SignatureAlgorithm(new SHA1(), new RSA(cert1.getManager().getProvider())), "ICPBravo", new Date(), "ICPBravo Test 1", "Brasil");
	        ...
	        sd2.verify();
			sd2.addSigner(cert2, new SignatureAlgorithm(new SHA1(), new RSA(cert2.getManager().getProvider())), "ICPBravo", new Date(), "ICPBravo Test Contra-Assinatura", "Brasil");
			...
			for (CMSSignerInformation signerInformation : signers) {
				...
				List&lt;Pkcs9Attribute&gt; signedAttributes = pkcs9SignedAttributes.getAttributes();
				...
				List&lt;Pkcs9Attribute&gt; unsignedAttributes = pkcs9UnsignedAttributes.getAttributes();
	...
	public static void testaAssinaturaDePDF() {
		try {
			...
			sd.addSigner(cert1, new SignatureAlgorithm(new SHA1(), new RSA(cert1.getManager().getProvider())), new Date(), "ICPBravo Test 1", "Brasil");
	        sd.signToFile("c:/temp/out.pdf"); 

	        // Abrindo o PDF assinado, verificando e adicionando uma nova assinatura
	        PDFSignedData sd2 = new PDFSignedData("c:/temp/out.pdf");
	        sd2.verify();
			sd2.addSigner(cert2, new SignatureAlgorithm(new SHA1(), new RSA(cert2.getManager().getProvider())), new Date(), "ICPBravo Test Contra-Assinatura", "Brasil");
	        sd2.signToFile("c:/temp/out2.pdf"); 
			...
			for (CMSSignerInformation signerInformation : signers) {
				...
				Pkcs9 pkcs9SignedAttributes = signerInformation.getSignedAttributes();
				List<Pkcs9Attribute> signedAttributes = pkcs9SignedAttributes.getAttributes();
				for (Pkcs9Attribute attribute : signedAttributes) {
					System.out.println(attribute.description() + ": " + attribute.toString());
				}
				Pkcs9 pkcs9UnsignedAttributes = signerInformation.getUnsignedAttributes();
				List<Pkcs9Attribute> unsignedAttributes = pkcs9UnsignedAttributes.getAttributes();
				for (Pkcs9Attribute attribute : unsignedAttributes) {
					System.out.println(attribute.description() + ": " + attribute.toString());
				}
	...
	public static void testaSignedData() {
		...
		try {
			...
			coAssinatura.addSigner(cert2, policy); // Adiciona um signatário ao pacote
			...
			contraAssinatura.addSigner(cert3, policy); // Adiciona um signatário ao pacote
			...
			multiplasAssinatura.addSigner(cert4, policy); // Adiciona um signatário ao pacote
...
Fim de teste de signedData (564 ms)
	...
	/**
	 * O teste abaixo demonstra uma situação muito comum.<br>
	 * Demonstra um caso onde se monta uma assinatura que exige dados oriundos de internet, como carimbos de tempo e LCRs, porém em um momento onde isto não é possível de se obter.<br>
	 * Neste caso, será necessário executar um procedimento posterior para inserir estes atributos na assinatura, respeitando-se o fato de só alterar os atributos que não foram assinados, de forma a não inutilizar a assinatura.<br>
	 * O código para este método é o seguinte:
	 * 
	 * <pre class="brush: java;">
	public static void testaCorrecaoSignedData() {
		try {
			cron.zera();
			System.out.println("Testando geração de signedData");
			
			// Informa a carimbadora
			TimestampRequester carimbadora = new TimestampRequester("http://tsp.iaik.at/tsp/TspRequest");
			
			// Informa a política utilizada. Neste caso, utilizaremos uma política AD-RT, cujo o atributo de carimbo de tempo é mandatório.
			Policy policy = new AdRTPolicy(carimbadora);
			
			// Neste ponto vamos indicar a API que nenhum acesso a internet poderá ser efetuado
			// Desta forma, ao criar a assinatura, mesmo que seja de uma política que exige carimbo de tempo, o mesmo não poderá ser recuperado
			ICPBravoUtil.setInternetAccess(false);

			// Criando uma nova assinatura
			CMSSignedData sd = new CMSSignedData();
			sd.setData("teste".getBytes());
			sd.setEncapsulate(true); // Attachment
			sd.addSigner(cert1, policy);
			
			// Neste ponto teremos então uma assinatura com indicação de política que informa que tem que ter carimbo de tempo, porém como não tivemos acesso a internet, não foi possível a inserção do carimbo e nem das LCRs.
			byte[] sdPkcs7CMS = sd.getASN1Encoded();
			
			// Recuperando a assinatura a partir dos bytes
			sd = new CMSSignedData(sdPkcs7CMS);
			try {
				// Neste caso, se verificássemos a consistência da política, notaríamos que seria lançada uma exceção indicando a falta dos atributos.
				sd.verifyPolicies();
			} catch (Exception e) {
				System.out.println(e.getMessage());
			}
			
			// Vamos indicar agora que já é possível obter acesso a internet ... 
			ICPBravoUtil.setInternetAccess(true);
			
			// ... e invocar então o método para tentar adicionar os atributos (não assinados) faltantes em nossa assinatura.
			sd.tryCorrect(carimbadora);
			sdPkcs7CMS = sd.getASN1Encoded();
			
			// Vamos realizar novamente os testes de integridade da política para avaliarmos a existência dos novos atributos
			sd = new CMSSignedData(sdPkcs7CMS);
			try {
				// Neste caso só será lançada exceção para o caso de não utilizar uma carimbadora ICP-Brasil.
				sd.verifyPolicies();
			} catch (Exception e) {
				System.out.println(e.getMessage());
			}
			
			cron.marcaTotal("Fim de teste de signedData com política RT");
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("SignedData RT: Falha " + e.getMessage());
		}
	}
	 * </pre>
	 */
	...
	public static void testaXmlDSig() {
		...
		try {
			...
			CMSSignedData sd = new CMSSignedData();
			sd.setData(new XMLContent("&lt;MY_XML&gt;&lt;Node1&gt;teste&lt;/Node1&gt;&lt;/MY_XML&gt;"));
			sd.setEncapsulate(true);
			sd.addSigner(cert1, policy);
			
			String xml = sd.getXMLEncoded("Node1");
	...
	public static void testaXmlDSig() {
		...
		try {
			...
			CMSSignedData sd = new CMSSignedData();
//			sd.setData(new XMLContent("<MY_XML><Node1 Id='1234'>teste</Node1></MY_XML>"));
			sd.setData(new FileContent("c:/temp/Tima/OAK-NFE-001.xml"));
			sd.setEncapsulate(true);
			sd.addSigner(cert1, policy);
			
//			String xml = sd.getXMLEncoded("Node1");
			String xml = sd.getXMLEncoded(XmlEncapsulated.Enveloping,  "infNFe");
			System.out.println(xml);
	 ...
	 * 
	 * 		CMSEnvelopedData sd = new CMSEnvelopedData(new AES128CBC());
	 * 		sd.setData(bts);
	 * 		sd.addKeyTransRecipient(cert1);
	 * 		byte[] sdPkcs7CMS = sd.getASN1Encoded();
	 * 
	 * 		sd = new CMSEnvelopedData(sdPkcs7CMS);
	 * 		if (Arrays.equals(sd.getContent(cert1), bts))
	...
	public static void testaEnvelopedData() {
		...
		try {
			...
			for (ICPBravoCertificate cert1 : certs) {
				// adiciona o recipiente para o destinatário (em resumo, criptografa a chave com a chave pública)
				sd.addKeyTransRecipient(cert1);
			}
			
//			sd.addKeyTransRecipient(cert1);
			// Gera o pacote, para que possa ser enviado ao(s) destinatários
			byte[] sdPkcs7CMS = sd.getASN1Encoded();
//			System.out.println(sd.getXMLEncoded());

			// Abre o pacote a partir dos bytes existentes
			sd = new CMSEnvelopedData(sdPkcs7CMS);
			if (Arrays.equals(sd.getContent(cert1), bts))
				System.out.println("EnvelopedData: OK");
			else
	 ...
	 * 
	 * 		SignedAndEnveloped sae = new SignedAndEnveloped(manager, cert1, destinatarios, new SignatureAlgorithm(new SHA1(), new RSA(cert1
	 * 				.getManager().getProvider())), new AES128CBC(), SignedAndEnveloped.SIGN_AND_ENVELOPED, new BytesContent(bts));
	 * 
...
Fim de teste de signedAndEnvelopedData (98 ms)
	...
	public static void testaSignedAndEnvelopedData() {
		...
		try {
			...
			SignedAndEnveloped sae = new SignedAndEnveloped(cert1, destinatarios, new SignatureAlgorithm(new SHA1(), new RSA(
					cert1.getManager().getProvider())), new AES128CBC(), SignedAndEnveloped.SIGN_AND_ENVELOPED, new BytesContent(bts));
	...
	public static void testaPBE() {
		...
		try {
			PBEAlgorithm pbe = new PBE_Sha1AndTripleDES112();
	...
	public static void testaPKCS12() {
		...
		try {
			...
			ICPBravoCertificateGenerator gen = new ICPBravoCertificateGenerator(mgr.getProvider(), aliasRoot, BigInteger.valueOf(1),
					...
					"Instituto Nacional de Tecnologia da Informacao fake - ITIf,L=Brasilia,S=DF", null, null, 
					from, to, 
					new SignatureAlgorithm(new SHA1(), new RSA1024ECB(mgr.getProvider())), 
					null);
...
Fim de teste de PKCS12 (25595 ms)
	...
	public static void testaLDAP() {
		...
		try {
			...
			String host = "ldap://directory.CertiSign.com.br";
			String name = "cn=FENACON Certisign RFB Resposta OCSP G2:01554285000175";
//			String name = "cn=CertiSign SRF Resposta OCSP V3";
			String credentials = "";
			String password = "";
...
//			manager.reload();
	 ...
	 * 
	 * 		message.addMailPart(new SignatureMailPart(manager, cert1, new SignatureAlgorithm(new SHA1(), new RSA(cert1.getManager()
	 * 				.getProvider())), part));
	 * 
	 * 		message.sendMessage();
	...
	public static void testaSMIME() {
		...
		try {
			...
			message.setSender("UbiratanElias@oaktech.com.br");
			MimeBodyPart part = message.addMailPart(new DataSourceMailPart(new TextEncodedDataSource("ttt", "Teste")));
			message.addMailPart(new SignatureMailPart(manager, cert1, new SignatureAlgorithm(new SHA1(), new RSA(cert1.getManager().getProvider())), part));
			message.sendMessage();
			System.out.println("S/MIME: OK");
		} catch (Exception e) {
			System.out.println("S/MIME: Falha " + e.getMessage());
	 ...
	 * 		SymmetricAlgorithm sym = new AES128ECB();
	 * 
...
//			ICPBravoManager mgr = new PKCS11OpenSCManager();
			...
			for (ICPBravoCertificate bravoCertificate : certs) {
				...
				SignatureAlgorithm sign = new SignatureAlgorithm(new SHA1(), new RSA(provider));
...
Fim de teste de Certificados em PEM (63 ms)
	...
	public static void testaCSR() throws Exception {
		...
		try {
			...
			SignatureAlgorithm sigAlg = new SignatureAlgorithm(new SHA1(), rsa);
			...
			ICPBravoCertificateGenerator gen = new ICPBravoCertificateGenerator(
					...
					"http://www.certificadodigital.oaks.com.br/repositorio/dpc", 
					from, to,  
					new SignatureAlgorithm(new SHA1(), new RSA(provider)), 
					"pessoa1S@oaktech.com.br", 
					ICPBravoUtil.getDate(2009, 1, 1), // data de nascimento
					"12345678901", // cpf
					null, // nis
...
//					new SignatureAlgorithm(new SHA1(), new RSA(provider)), 
			...
			dispositivo.install(alias, null, null, certp7BPEM);
	...
	/**
	 * Testa a capacidade de receber uma data/hora de um servidor de data/hora, via network time protocol
	 * <p>
	 * <pre class="brush: java;">
	public static void testaNTP() throws Exception {
		cron.zera();
		System.out.println("Testando NTP");
		try {
			NtpClient ntp = new NtpClient("time.windows.com"); // "clock.psu.edu"
			System.out.println(ntp.toString());
			System.out.println("NTP: OK");
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("NTP: : Falha " + e.getMessage());
		}
		cron.marcaTotal("Fim de teste de NTP");
	}
	 * </pre>
	 * 
	 * O método apresenta como saída as seguintes informações:<br />
	...
	/**
	 * Testa uma transação de challenge/response<br>
	 * É possível utilizar a facilidade de challenge/response de duas formas:<br>
	 * Na primeira, o servidor inicia o processo enviando um desafio, sem a necessidade de saber quem é o destinatário.<br>
	 * Na segunda, o cliente informa quem é o destinatário (cert), onde o desafio é criptografado com a chave pública do mesmo, 
	 * sendo que o cliente deverá, antes de responder ao desafio, decriptá-lo com sua chave privativa.<br>
	 * No exemplo abaixo, que demonstra a segunda opção, para transformá-lo para a primeira, basta utilizar o construtor de
	 * ICPBravoCRServer que não recebe o certificado como parâmetro.
	 *  
	 * <p>
	 * <pre class="brush: java;">
	public static void testaChallengeResponse() throws Exception {
		cron.zera();
		System.out.println("Testando Challenge/Response");
		try {
			// Servidor cria desafio
			ICPBravoCRServer server = new ICPBravoCRServer(new AES128CBC(), cert1);
			ICPBravoChallenge challengeServer = server.createChallenge();
			byte [] challengeBts = challengeServer.getASN1Encoded();
			
			// Cliente abre o desafio e assina, gerando a resposta
			ICPBravoChallenge challengeClient = new ICPBravoChallenge(challengeBts);
			ICPBravoResponse responseClient = challengeClient.getResponse(cert1); 
			byte [] responseBts = responseClient.getASN1Encoded();
			
			// Servidor abre o desafio e verifica
			ICPBravoResponse responseServer = new ICPBravoResponse(responseBts);
			server.verify(challengeServer, responseServer);
			
			System.out.println("Challenge/Response: OK");
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("Challenge/Response: : Falha " + e.getMessage());
		}
		cron.marcaTotal("Fim de teste de Challenge/Response");
	}
	 * </pre>
	 * 
	 * O método apresenta como saída as seguintes informações:<br />
	...
	public static void main(String[] args) throws Exception {
		...
		criaCertificados();
		System.out.println("testaChallengeResponse() - ######################################################################");
		testaChallengeResponse();
...
//		testaAssinaturaDePDF();
		...
		changeTrustedChain();
		System.out.println("testaCorrecaoSignedData() - ######################################################################");
		testaCorrecaoSignedData();
		System.exit(0);
	}
}