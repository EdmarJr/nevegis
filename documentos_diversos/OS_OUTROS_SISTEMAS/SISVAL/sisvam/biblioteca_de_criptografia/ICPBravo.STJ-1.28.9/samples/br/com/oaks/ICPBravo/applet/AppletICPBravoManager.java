...
public class AppletICPBravoManager extends AppletICPBravoBase {
	...
	/**
	 * Indica se o Applet funcionará em modo Callback, onde todos os retornos se darão por meio de callback ou não
	 * @param mode - true (default) - retorno por meio de callback, false - retorno direto pelo método
	 */
			...
			public Void run() {
				try {
					...
					try {
						...
						ret += "Assinatura ICP-Brasil não OK";
					}
...
//		return alg.sign(sig.getContent(), cert);
				...
				try {
					...
					if (_callBackMode)
						if (sign != null)
							...
							showError(null, "Certificado inválido");
	...
	private byte [] signMessage(final int alias, final Content message, final Content signature, final boolean encapsulate) throws Exception {
		...
		if (_currentConfiguration.getConf().podeAssinar(cert)) {
			...
			try { // Se já tem uma assinatura
				cmsSignature = new CMSSignedData(signature);
			} catch (Exception e) { // Se é uma assinatura nova
...
//						"ICPBravo",
	...
	private int getCertificateSelected(final String manager) throws Exception {
		...
		certs.setToolTipText("Selecione o certificado que deverá ser utilizado para a operação");
